...
class GatorPrism(nn.Module):
        ...
        self.expert_norms = nn.ModuleDict(
            {
                ...
                for name in self.coalition_names
            }
        )
    ...
    def forward(
        ...
        normalized_coalition_embeddings = {
            name: self.expert_norms[name](coalition_embeddings[name])
            for name in self.coalition_names
        }
        normalized_coalition_stack = torch.stack(
            [
                normalized_coalition_embeddings[name]
                for name in self.coalition_names
            ],
            dim=1,
        )
        base = normalized_coalition_stack.mean(dim=1)
        ...
        fused = (
            normalized_coalition_stack * gates.unsqueeze(-1)
        ).sum(dim=1)
        ...
        return {
            ...
            "normalized_coalition_embeddings": normalized_coalition_embeddings,
            "normalized_coalition_stack": normalized_coalition_stack,
