"""Core GatorPrism model and self-supervised objectives.

This module implements the method specified in ``GatorPrism_Methods.md``:

* one joint graph-attention expert on the cross-omics consensus graph;
* one modality-specific graph-attention expert per modality on its mixed graph;
* Student-t associations to trainable routing prototypes;
* a prototype-conditioned, spot-specific coalition router;
* residual expert fusion followed by layer normalization;
* modality-aware reconstruction, graph-neighborhood contrastive learning,
  cross-modal NT-Xent alignment, spatial smoothness, and load balancing.

The routing prototypes are internal routing context.  No DEC target
distribution, clustering KL loss, or annotation supervision is used.
"""
...
def _two_layer_mlp(
    ...
    """Two-layer MLP used by the router, encoders, and decoders."""
...
class MultiHeadGraphAttention(nn.Module):
    """A dependency-light multi-head graph-attention layer.

    The implementation uses native PyTorch scatter operations, so the project
    does not require ``torch-scatter`` or ``torch-geometric``.  Incoming
    messages are normalized independently for each destination node and head.
    """
...
class CoalitionGraphExpert(nn.Module):
    """A separate graph-attention encoder for one omics coalition."""
...
class GatorPrism(nn.Module):
    """Prototype-conditioned coalition graph mixture-of-experts.

    Parameters
    ----------
    modality_dims
        Ordered mapping from modality name to processed input dimension.
    n_prototypes
        Number ``R`` of trainable routing prototypes.  This is deliberately
        independent of the final number ``K`` of spatial domains.
    hidden_dim
        Common latent dimension ``d`` of every coalition expert.
    alpha
        Degrees-of-freedom parameter of the Student-t routing kernel.
    gate_temperature
        Softmax temperature ``tau`` of the coalition router.
    """
    ...
    def prototype_association(self, base: Tensor) -> Tensor:
        """Student-t association ``q_ir`` between spots and prototypes."""
...
class LossWeights:
    """Coefficients in the joint GatorPrism objective."""
...
def sample_distinct_non_edges(
    ...
    """Uniformly sample distinct ordered non-edges without replacement.

    Self-pairs and every positive ordered pair are excluded.  By default the
    number of negatives equals the number of distinct positive edges, exactly
...
def reconstruction_loss(
    ...
    """Sum of modality MSEs, each normalized by ``N * d_m``."""
...
def edge_contrastive_loss(
    ...
    """Logistic contrastive loss on graph edges and matched non-edges."""
...
def symmetric_nt_xent(
    ...
    """Symmetric cross-modal NT-Xent for co-registered spots."""
...
def spatial_smoothness_loss(z: Tensor, spatial_edges: Tensor) -> Tensor:
    """Mean graph Dirichlet energy over the spatial edge set."""
...
def load_balance_loss(gates: Tensor) -> Tensor:
    """Squared deviation of aggregate coalition usage from ``1 / C``."""
...
def compute_gatorprism_loss(
    ...
    """Compute the complete self-supervised GatorPrism objective."""
...
__all__ = [
    "CoalitionGraphExpert",
    "GatorPrism",
    "LossWeights",
    "MultiHeadGraphAttention",
    "compute_gatorprism_loss",
    "cross_modal_alignment_loss",
    "edge_contrastive_loss",
    "load_balance_loss",
    "reconstruction_loss",
    "sample_distinct_non_edges",
